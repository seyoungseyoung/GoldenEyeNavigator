...
// const REPOSITORY_NAME = 'YOUR_REPOSITORY_NAME'; // 로컬 개발 시 불필요하므로 주석 처리

const isProd = process.env.NODE_ENV === 'production';

const nextConfig: NextConfig = {
  // output: 'export', // 정적 사이트 익스포트를 위해 추가 -> 개발 중에는 주석 처리
  // basePath: isProd ? `/${REPOSITORY_NAME}` : '', // 로컬 개발 시 불필요
  // assetPrefix: isProd ? `/${REPOSITORY_NAME}` : '', // 로컬 개발 시 불필요
  images: {
